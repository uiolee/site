...
本文将使用 [GitHub Actions](https://docs.github.com/zh/actions) 部署至 GitHub Pages，此方法适用于公开或私人储存库。 它在公共和私人仓库中工作。 It works in both public and private repository. 若你不希望将源文件夹上传到 GitHub，请参阅 [一键部署](#一键部署)。
...
3. 使用 `node --version` 指令检查你电脑上的 Node.js 版本，并记下该版本 (例如：`v16.y.z`) Make a note of the major version (e.g., `v16.y.z`) 给主要版本注解(例如， `v16.y.z`
...
1. 前往 `https://<你的 GitHub 用户名>.github.io` 查看网站。 转到 **设置** 标签页。 Go to the **Settings** tab. 建立名为 `<repository 的名字>` 的储存库，这样你的博客网址为 `<你的 GitHub 用户名>.github.io/<repository 的名字>`，repository 的名字可以任意，例如 blog 或 hexo。
