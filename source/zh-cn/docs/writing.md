...
Hexo 有三种默认布局：`post`、`page` 和 `draft`。 每个文件创建的文件被保存到不同的路径。 新创建的帖子被保存到 `源/_post` 文件夹。
...
在新建文章时，Hexo 会根据 `scaffolds` 文件夹内相对应的文件来建立文件，例如： 例如：

``` bash
$ 十六进制新照片"My Gallery
```

在执行这行指令时，Hexo 会尝试在 `scaffolds` 文件夹中寻找 `photo.md`，并根据其内容建立文章，以下是您可以在模版中使用的变量： 以下占位符可用于scaffolds：
