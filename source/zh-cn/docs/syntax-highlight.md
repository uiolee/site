...
> 提示：Hexo 支持用任何格式书写文章，只需安装相应渲染插件即可。 It can be in markdown, ejs, swig, nunjucks, pug, asciidoc, etc. Regardless of the format used, those three code block syntax will always be available. 无论使用何种格式，这三个代码块语法将永远可用。
## 配置
...
Replace tabs inside code block with given string. By default it is 2 spaces. 默认情况下为 2 个空格。
...
All prism plugins are supported if `preprocess` is set to `false`. Here are a few things you should still pay attention to: 以下是你仍然应该注意的几件事：
...
Replace `\t` inside code block with given string. By default it is 2 spaces. 默认情况下为 2 个空格。
