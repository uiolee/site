...
| 變數                 | 描述                                            | Type                     |
| ------------------ | --------------------------------------------- | ------------------------ |
| `page.title`       | 文章標題                                          | `文章標籤`                   |
| `page.date`        | Article created date                          | 文章更新日期（[Moment.js][] 物件） |
| `page.updated`     | Article last updated date                     | 文章建立日期（[Moment.js][] 物件） |
| `page.comments`    | Comment enabled or not                        | `boolean`                |
| `page.layout`      | Layout name                                   | `string`                 |
| `page.content`     | 文章的完整網址                                       | `string`                 |
| `page.excerpt`     | 文章的原始內容                                       | `彙整月份（不含開頭的零）`           |
| `page.more`        | 除了文章摘要的其餘內容                                   | `總頁數`                    |
| `page.source`      | The path of the source file                   | `category`               |
| `page.full_source` | Full path of the source file                  | `tag`                    |
| `page.path`        | 文章網址（不含根路徑）。 我們通常在主題中使用 `url_for(page.path)`。 | `string`                 |
| `page.permalink`   | 文章的完整原始路徑                                     | `string`                 |
...
| `page.link`        | 文章的外連連結（用於連結文章）                               | `留言是否開啟`                 |
...
| `page.next_link`   | 上一頁的連結。 如果此頁是第一頁的話則為 `''`。                                             | `文章原始路徑`  |
| `page.path`        | 目前頁面的路徑（不含根目錄）。 我們通常在主題中使用 `url_for(page.path)`。                       | `文章的完整內容` |
