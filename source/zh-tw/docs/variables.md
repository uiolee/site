...
| `page.content`     | 文章的完整網址                                       | `總頁數`                    |
| `page.excerpt`     | 文章的原始內容                                       | `文章的完整內容`                |
| `page.more`        | 除了文章摘要的其餘內容                                   | `category`               |
| `page.source`      | The path of the source file                   | `彙整月份（不含開頭的零）`           |
| `page.full_source` | Full path of the source file                  | `文章標籤`                   |
| `page.path`        | 文章網址（不含根路徑）。 我們通常在主題中使用 `url_for(page.path)`。 | `#網站變數`                  |
